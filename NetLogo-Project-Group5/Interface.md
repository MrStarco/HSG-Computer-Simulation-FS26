...
This document lists the current interface state from `Group5_Misconduct_ABM.nlogox`.

## 1) Buttons

1. `setup`
   - Command: `setup`
   - Type: once (`forever = false`)

2. `go`
   - Command: `go`
   - Type: forever (`forever = true`)
...
9. `Hidden misconduct (tick)` -> `hidden-misconduct-this-tick`
10. `Hidden misconduct rate (tick)` -> `hidden-misconduct-rate-this-tick` (precision `3`)
11. `Reported events (tick)` -> `reported-events-this-tick`
12. `Retaliation events (tick)` -> `retaliation-events-this-tick`
...
### Plot 2: `Relative Misconduct Change (%)`
...
## 5) Hardcoded constants (not sliders)
...
## 6) Embedded BehaviorSpace experiments

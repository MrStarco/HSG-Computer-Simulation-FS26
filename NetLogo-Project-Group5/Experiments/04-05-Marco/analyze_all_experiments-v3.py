...
EXP2_CSVS = {
    ...
    "response-strength":             _resolve_csv_path(f"{PREFIX}exp2d_sens_response_strength-table.csv"),
    "drift-speed":                   _resolve_csv_path(f"{PREFIX}exp2e_sens_drift_speed-table.csv"),
}
...
def load_csv(path):
    ...
    required_cols = {"run", "number-employees", "punishment-value", "reporter-protection"}
...
def load_spreadsheet_csv(path):
    ...
    for i in range(n_cols):
        ...
        # Parameters (e.g., punishment-value, reporter-protection, number-employees)
...
# Each tuple: (punishment-value, reporter-protection, short label, description)
ARCHETYPES = [
    (0.20, 0.20, "Status Quo",
     "P=0.20 / R=0.20\nWeak deterrence, exposed reporters"),
    (0.80, 0.20, "Iron Fist",
     "P=0.80 / R=0.20\nHigh punishment, low protection — fear trap"),
    (0.20, 0.80, "Safe Harbor",
     "P=0.20 / R=0.80\nProtected reporters, low deterrence"),
    (0.80, 0.80, "Rule of Law",
     "P=0.80 / R=0.80\nStrong deterrence + full protection — sweet spot"),
]
...
def _plot_exp1_mechanism(df):
    ...
    x_levels = sorted(np.round(means["punishment-value"].dropna().unique(), 2))
...
def _plot_exp1_pareto(df):
    ...
    cells = (
        df.groupby(["punishment-value", "reporter-protection"])[
    ...
    def scatter_pareto(ax, x, y, xlab, ylab, title):
        ...
        for r in level_vals:
            rows = cells[np.isclose(cells["reporter-protection"], r, atol=1e-6)].sort_values("punishment-value")
            if len(rows) > 1:
    ...
    axes[0].legend(
        handles=size_handles, title="punishment-value",
...
def _plot_exp1_archetypes(df):
    ...
    for P_val, R_val, label, desc in ARCHETYPES:
        rows = df[
            (df["punishment-value"].sub(P_val).abs() < 0.001) &
...
EXP2_PARAMS = [
    ...
    ("response-strength",             "response-strength",              0.20),
